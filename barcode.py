...
while True:
    ...
    for barcode in decode(area):
        print(barcode.data)
        mydata = barcode.data.decode('utf-8')
        print(mydata)
        pts = np.array([barcode.polygon],np.int32)
        pts = pts.reshape((-1,1,2))
        cv2.polylines(area,[pts],True,(255,255,0),5)
        pts2 = barcode.rect
        cv2.putText(area, mydata,(pts2[0],pts2[1]),cv2.FONT_HERSHEY_COMPLEX,0.9,(255,0,255),2)
    cv2.imshow('Result',area)
    #cv2.imshow("Laplacian",area)
    cv2.waitKey(1)